...
type SourceSelection = M8Source | `channel:${string}`;
...
function Home() {
  ...
  const channelsQuery = useQuery({
    queryKey: ["youtube-channels", savedToken],
    queryFn: () => fetchYoutubeChannels(savedToken),
    enabled: isPublicSearch || savedToken.length > 0,
  });
  ...
  const error = errorMessage(
    statsQuery.error ?? channelsQuery.error ?? searchMutation.error,
  );
                ...
                {(field) => (
                  <div className="flex flex-col gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        if (value) {
                          field.handleChange(value as SourceSelection);
                        }
                      }}
                    >
                          ...
                          <>
                            ...
                            <SelectGroup>
                              ...
                              {channels.map((channel) => (
                                <SelectItem
                                  key={channel.name}
                                  value={channelSelection(channel.name)}
                                >
                                  <span className="flex flex-1 items-center justify-between gap-4 mr-5">
...
async function fetchYoutubeChannels(
  ...
  return response.json() as Promise<YoutubeChannelsResult>;
}
...
async function searchM8(
  ...
  const channel = channelFromSelection(request.source);
  const response = await fetch("/api/search", {
    ...
    body: JSON.stringify({
      ...
      sources: [channel ? "video" : request.source],
...
function channelSelection(channel: string): SourceSelection {
  return `channel:${channel}`;
}

function channelFromSelection(selection: SourceSelection): string | undefined {
  return selection.startsWith("channel:")
    ? selection.slice("channel:".length)
    : undefined;
}

function sourceSelectionLabel(
  selection: SourceSelection,
  channels: YoutubeChannelsResult["channels"],
): string {
  const channelName = channelFromSelection(selection);
  if (channelName) {
    const channel = channels.find(
      (candidate) => candidate.name === channelName,
    );
    return channel ? `${channel.name} (${channel.videos})` : channelName;
  }

  return (
    sourceOptions.find((option) => option.value === selection)?.label ??
    selection
  );
}
